<template>
    <div class="transfer">
        ...
        <div class="btns">
            <el-select style="margin:140px 0 0 15px;width:145px" v-model="premiumAll" size="small" placeholder="请选择">
                <el-option label="不加价" :value="0"></el-option>
                <el-option label="比例加价" :value="1"></el-option>
                <el-option label="固定加价" :value="2"></el-option>
            </el-select>
            <!--  -->
            <div style="margin:10px 0 0 0;font-size:18px;font-weight:600;text-align:center" v-show="!(premiumAll==0)">
                <span>加价</span>
                <el-input-number v-model="addPrice" size="mini" style="width:100px;vertical-align: middle"></el-input-number>
            </div>
            <!--  -->
            <el-button style="margin:10px 0 0 55px;" type="primary" size="small" @click="addFootList">添加 <i class="el-icon-d-arrow-right"></i></el-button>
            <br>
        ...
        <div class="right">
            ...
            <div class="footComList">
                <div class="header_tops">
                    <div class="tit">菜品名称</div>
                    <div class="tit" style="width:22%">加价方式</div>
                    <div class="tit" style="width:26%">加价金额/比例</div>
                    &nbsp;
                    <div class="tit" style="width:18%">加价后价格/元</div>
                    <div class="tit" style="width:8%">操作</div>
                </div>
                <!-- 滚动内容 -->
                <div class="content_scroll">
                    <div v-for="(v,k) in addList" :key="k">
                        <div class="border_bottom">
                            ...
                            <div class="tit" style="width:22%">
                                <el-select size="small" v-model="v.premium" placeholder="请选择" @change="premiumCHange(1,k)">
                                    <el-option label="不加价" :value="0"></el-option>
                                    <el-option label="比例加价" :value="1"></el-option>
                                    <el-option label="固定加价" :value="2"></el-option>
                                </el-select>
                            </div>
                            <div class="tit" style="width:26%">
                                <el-input-number style="width:78%" size="mini" :disabled="v.premium==0" :min="0" v-model="v.price" @change="premiumCHange(2,k)"></el-input-number>
                                <span v-if="v.premium==0||v.premium==2">元</span>
                                <span v-else>%</span>
                            </div>
                            &nbsp;
    ...
    export default {
        props:{
            selectedList:{
                default(){
                    return [//默认已经添加的食品
                        {id: "1",money: 21,viPrice:33,name: "好吃的饺子",type: 1,premium:0,price:0},
                        {id: "5",money: 21,viPrice:33,name: "12米面条",type: 1,premium:0,price:0},
                    ]
                }
            },
            footClass:{
        ...
        methods:{
            ...
            premiumCHange(e,k){
                // e    1    加价方式    2   加价金额/比例
                var premium=this.addList[k].premium//加价方式   0  不加价   1   比例加价   2  固定加价
                var defaultMoney=this.getMoney(this.addList[k].id)  //获取初始价格
                if(e==1){
                    this.addList[k].price=0;
                    this.addList[k].money=defaultMoney
                }else{
                    if(premium==0){
                        // 获取原始价格
                        this.addList[k].money=defaultMoney
                    }else if(premium==1){
                        this.addList[k].money=defaultMoney+defaultMoney*this.addList[k].price/100
                    }else{
                        this.addList[k].money=defaultMoney+this.addList[k].price
                    }
                }
                // 驱动视图更新
                this.index++
            ...
            addFootList(){
                var array=[];
                var way=this.premiumAll;  // 0  不加价  1  比例加价   2  固定加价固定加价
                var num=this.addPrice;  //  加价金额/比例
                var money_s=0;
                for(var i=0,len=this.forList.length;i<len;i++){
                    if(this.forList[i].check&&!this.forList[i].desable){
                        if(way==0){
                            money_s=this.forList[i].money;
                        }else if(way==1){
                            money_s=parseFloat((this.forList[i].money+this.forList[i].money*num/100).toFixed(2))
                        }else{
                            money_s=this.forList[i].money+num
                        }
                        array.push({
