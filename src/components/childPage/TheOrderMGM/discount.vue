            ...
            <div class="tabel_list_body">
                <table class="table_list table_list_box" border="0" cellspacing="0">
                    ...
                    <tr v-for="(v,k) in data_list" :key="k">
                        ...
                        <td>{{v.code}}</td>
                        <td>{{v.Seat}}</td>
                        <td>{{v.originallyMoney}}</td>
                        <td>{{v.actualMoney}}</td>
                        <td>{{v.discountMoney}}</td>   
                        <td>{{v.allOrder}}</td>
                        <td>{{v.plan}}</td>
                        <td>{{v.dishes}}</td>
                        <td>{{v.user}}</td>
            ...
            return {
                ...
                data_list:[
                    {   name:'安少爷米线（商之都店）',why:'客户结账后退菜',code:'2333445534',Seat:'PSHJD2',
                        originallyMoney:215,actualMoney:200,discountMoney:15,allOrder:0,plan:0,dishes:2,
                        user:'去你妈的',pointTime:'2018-12-23 11:12:45'
                    },
                    {   name:'安少爷米线（商之都店）',why:'客户结账后退菜',code:'2333445534',Seat:'PSHJD2',
                        originallyMoney:215,actualMoney:200,discountMoney:15,allOrder:0,plan:0,dishes:2,
                        user:'去你妈的',pointTime:'2018-12-23 11:12:45'
            ...
            ExportData(){
                this.$confirm('此操作将导出全部菜品数据, 是否继续?', '提示', {
                    ...
                    require.ensure([], () => {
                        ...
                        const filterVal = ['name', 'why', 'code', 'Seat','originallyMoney','actualMoney','discountMoney','allOrder','plan','dishes','user','pointTime'];
